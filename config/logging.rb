Logging::Rails.configure do |config|
  ...
  Logging.init %w[debug info warn error fatal audit]

  # Objects will be converted to strings using the :inspect method.
  Logging.format_as :inspect

  # The default layout used by the appenders.
  layout = Logging.layouts.pattern(:pattern => '[%X{ip_address}:%X{session_id}:%X{user_id}:%d] %-5l %c : %m\n')
  ...
  Logging.appenders.stdout( 'stdout',
    ...
    :layout => Logging.layouts.pattern(
      :pattern => '[%X{ip_address}:%X{session_id}:%X{user_id}:%d] %-5l %c : %m\n',
      :color_scheme => 'bright'
    )
  ...
  Logging.appenders.rolling_file( 'debug',
    ...
    :level => :debug,
    :layout => layout
  ) if config.log_to.include? 'debug'

  Logging.appenders.rolling_file( 'audit',
                                  :name => 'audit_appender',
                                  :filename => 'log/chorus_audit.log',
                                  :keep => 7,
                                  :age => 'daily',
                                  :truncate => false,
                                  :auto_flushing => true,
  ...
  Logging.appenders.file( 'error',
                                  ...
                                  :level => :error,
                                  :layout => layout
  ) if config.log_to.include? 'error'
